...
const updateFileOnExternalService = async (oldFileUrl, newFile) => {
  try {
    ...
    // If there is no old file to update or if it's not a URL from our service, just upload the new file
    if (!oldFileUrl || !oldFileUrl.includes(BASE_URL)) {
      return await uploadToExternalService(newFile);
    }
...
const deleteFileFromExternalService = async (fileUrl) => {
  try {
    if (!fileUrl || !fileUrl.includes(BASE_URL)) return;
    ...
    console.error('External delete error:', error.message);
